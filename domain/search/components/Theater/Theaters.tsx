...
export default function Theaters({
  ...
  const goChat = (event: React.MouseEvent<SVGSVGElement, MouseEvent>) => {
    ...
    <>
      {list.length > 0 && type === 'FACILITY' ? (
        ...
        dark:rounded-xl dark:border-2 dark:border-gray-100  relative">
              <Theater theater={theater} show={'list'} setList={setList} type={type} />
              <svg
                onClick={goChat}
        ...
        <ul>
          ...
          rounded-xl mb-2 border-2 border-gray-100 
        dark:rounded-xl dark:border-2 dark:border-gray-100  relative">
              <Theater theater={theater} show={'fac'} setList={setList} type={type} />
              {type === 'PERFORMANCE' ? (
                <svg
                  onClick={goChat}
                  xmlns="http://www.w3.org/2000/svg"
